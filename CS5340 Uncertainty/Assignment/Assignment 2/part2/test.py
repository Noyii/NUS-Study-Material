...
def check_answers(answer_dir = 'data/answers', pred_dir = 'data/predictions', test_range = None):
    ...
    for i in range(len(file_tuples) if test_range == None else test_range+1):
        # subprocess.run(f'python .\main.py --case {i+1}', capture_output=True)
        ...
        for j in range(len(keylist)):
            temp_keys = answers[j].keys()
            cur_answers = np.array([answers[j][key] for key in temp_keys])
            cur_preds = np.array([preds[j][key] for key in temp_keys])
            if np.allclose(cur_answers, cur_preds):
                match.append(True)
            else:
                match.append(False)
        if(np.all(match)):
