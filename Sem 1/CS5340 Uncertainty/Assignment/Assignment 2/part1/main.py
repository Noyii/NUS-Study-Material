...
def _update_mrf_w_evidence(all_nodes, evidence, edges, factors):
    ...
    for factor in factors:
        # Update each factor with evidence
        new_factor = factor_evidence(factor, evidence)
        if not new_factor.is_empty():
            updated_factors.append(new_factor)

    evidence_vars = list(evidence.keys())
    for e in evidence_vars:
        # Remove variable frpm query nodes if its an evidence
        idx = np.where(query_nodes == e)[0]
        query_nodes = np.delete(query_nodes, idx)
    
    updated_edges = []
    for e in edges:
        # Keep only those edges that don't connect evidences
...
def _get_clique_potentials(jt_cliques, jt_edges, jt_clique_factors):
    ...
    for i in range(len(jt_cliques)):
        # Assign clique potential to each node
        G.add_node(i, factor=jt_clique_factors[i])
    for edge in jt_edges:
        G.add_edge(edge[0], edge[1])

    clique_potentials = []
    # Get all individual junction trees if its a junction forest
    graphs = [G.subgraph(c) for c in nx.connected_components(G)]

    for graph in graphs:
        # Perform sum-product on individual junction tree
        cp = _sum_product(graph, jt_cliques, root=list(graph.nodes())[0])

        # get clique potentials for all junction trees
...
def _get_node_marginal_probabilities(query_nodes, cliques, clique_potentials):
    ...
    for i, var in enumerate(query_nodes):
        ...
        for j in range(len(cliques)):
            # Find the smallest clique which has the query variable
        ...
        # Marginalize all the variables in the clique except the query
