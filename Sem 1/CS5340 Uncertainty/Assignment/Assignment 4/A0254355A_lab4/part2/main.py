...
def _sample_step(nodes, factors, in_samples):
    ...
    for node in nodes:
        ...
        # Find probability of node given evidence of its markov blanket
...
def _get_conditional_probability(nodes, edges, factors, evidence, initial_samples, num_iterations, num_burn_in):
    ...
    """ YOUR CODE HERE """
    # Update the factors for each node by marginalizing with every variable not in its markov blanket
    ...
    # Ignore the samples created in burn_in period
    ...
    for _ in range(num_iterations):
        ...
        # Count occurrence of all samples
        samples_count[s] += 1
