...
def fitness_func(solution, sol_idx):
    ...
    #solution_fitness = result['f0.5'].cpu().detach().numpy() + 0.00000001
    
    predictions = model(data_inputs)
    abs_error = loss_function(torch.squeeze(predictions), data_outputs).cpu().detach().numpy() + 0.00000001
...
def ga(train_dataset, model_path):
    print("enter GA")
    global data_inputs
    global data_outputs
    data_inputs = torch.tensor(train_dataset.data, dtype=torch.float).to(device)
    data_outputs = torch.tensor(train_dataset.labels, dtype=torch.float).to(device)
    num_generations = 1000
    num_parents_mating = 5
    global torch_ga
    torch_ga = pygad.torchga.TorchGA(model=model, num_solutions=10)
    initial_population = torch_ga.population_weights
    parent_selection_type = "tournament" #"sss", "rws", "sus", "rank", "random", "tournament"
    crossover_type = "uniform" # "single_point", "two_points", "uniform", "scattered"
    mutation_type = "swap" #"swap", "inverse", “random", "scramble"
    mutation_percent_genes = 10 # Percentage of genes to mutate. This parameter has no action if the parameter mutation_num_genes exists.
    keep_elitism=3 #It accepts an integer to define the number of elitism (i.e. best solutions) to keep in the next generation.
    ...
    print("Crossentropy : ", loss_function(torch.squeeze(predictions), data_outputs).cpu().detach().numpy())

    # Calculate the classification accuracy for the trained model.
    accuracy = torch.sum(torch.max(predictions, axis=1).indices == data_outputs) / len(data_outputs)
    print("Accuracy : ", accuracy.cpu().detach().numpy())
    checkpoint = {
