...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mel_spectrogram_file = './resources/' + audio_fp.split('/')[3] + '_tensor.pt'

        if os.path.isfile(mel_spectrogram_file):
            mel_spectrogram = torch.load(mel_spectrogram_file)
        else:
            waveform, sample_rate = torchaudio.load(audio_fp)
            resampler = torchaudio.transforms.Resample(orig_freq=sample_rate, new_freq=self.sampling_rate, dtype=waveform.dtype)
            resampled_waveform = resampler(waveform)
            resampled_waveform = torch.mean(resampled_waveform, dim=0)

            transform = torchaudio.transforms.MelSpectrogram(sample_rate=self.sampling_rate, n_mels=256, n_fft=1024, hop_length=320)
            mel_spectrogram = transform(resampled_waveform)  # (channel, n_mels, time)
        ...
        start_frame = math.ceil(start_sec * self.frame_per_sec)
        end_frame = math.ceil(end_sec * self.frame_per_sec)

        spectrogram_clip = torch.swapaxes(mel_spectrogram[:, start_frame : end_frame], 0, 1)
    ...
    def get_labels(self, annotation_data, duration):
        ...
        for (start_sec, end_sec, pitch) in annotation_data:
            ...
            octave_roll[start_frame: end_frame] = octave
            pitch_roll[start_frame: end_frame] = pitch_class
