...
class LossFunc:
    def __init__(self, device):
        ...
        '''
        threshold = torch.tensor([15.0], device=device)
        self.onset_criterion = nn.BCEWithLogitsLoss(pos_weight=threshold)
        self.offset_criterion = nn.BCEWithLogitsLoss(pos_weight=threshold)    
    ...
    def get_loss(self, out, tgt):
        ...
        onset_loss = self.onset_criterion(on_out, on_tgt.squeeze().float())
        offset_loss = self.offset_criterion(off_out, off_tgt.squeeze().float())
        octave_loss = self.octave_criterion(torch.swapaxes(octave_out, 1, 2), octave_tgt)
        pitch_loss = self.pitch_criterion(torch.swapaxes(pitch_class_out, 1, 2), pitch_class_tgt)
...
class Metrics:
    ...
    def update(self, out, tgt, losses=None):
        ...
        with torch.no_grad():
            ...
            out_oct = torch.argmax(torch.softmax(out_oct, dim=2).cpu(), dim=2)
            out_pitch = torch.argmax(torch.softmax(out_pitch, dim=2).cpu(), dim=2)
            tgt_oct = tgt_oct.cpu()
            tgt_pitch = tgt_pitch.cpu()

            tgt_on = torch.flatten(tgt_on, 0).cpu().numpy()
            tgt_off = torch.flatten(tgt_off, 0).cpu().numpy()
            out_on = torch.flatten(out_on, 0).cpu().numpy()
            out_off = torch.flatten(out_off, 0).cpu().numpy()
            ...
            onset_f1 = f1_score(tgt_on, out_on)
            offset_f1 = f1_score(tgt_off, out_off)
            oct_acc = torch.sum(out_oct == tgt_oct) / (tgt_oct.shape[0] * tgt_oct.shape[1])
            pitch_acc = torch.sum(out_pitch == tgt_pitch) / (len(tgt_pitch) * len(tgt_pitch[0]))
