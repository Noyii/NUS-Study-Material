...
class ValueIterationAgent(object):
    ...
    def solve_value_iteration(self):
        ...
        value_policy = np.zeros(len(self.disc_states))
        policy_function = np.zeros(len(self.disc_states), dtype=np.int64)
        ...
        for _ in range(self.max_iterations):
            ...
            for s in range(len(self.disc_states)):
                # Initialize max value and best action for this state
                max_value = -np.inf
                best_action = None

                # Iterate over all actions
                for a in range(len(self.disc_actions)):
                    # Calculate expected value for this action
                    expected_value = 0
                    for prob, s_prime, reward, done in self.Prob[s][a]:
                        expected_value += prob * (reward + self.gamma * value_policy[s_prime] * (not done))
                ...
                # Update value function and policy for this state
                delta = max(delta, abs(max_value - value_policy[s]))
                value_policy[s] = max_value
